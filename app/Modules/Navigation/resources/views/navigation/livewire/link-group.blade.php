...
<div x-data="{
    ...
    <div @click="open = !open" class="flex cursor-pointer items-center"
        ...
        <div x-show="!open">
            <x-feather-icon x-show="open" name="chevron-right" class="h-5 w-5 stroke-2"/>
        </div>
        <div x-show="open">
            <x-feather-icon name="chevron-down" class="h-5 w-5 stroke-2"/>
        </div>

        <x-feather-icon name="folder" class="h-5 w-5 stroke-2 text-primary-darker"/>
